...
import torch
from utils.funcs import box_cxcywh_to_xyxy, generalized_box_iou
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher=None, weight_dict={'loss_ce': 1, 'loss_bbox': 5, 'loss_giou': 2}, eos_coef=0.1, losses=['labels', 'boxes']):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        ...
        loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight.to(target_classes.device))
    ...
    def forward(self, outputs, targets):
        ...
        loss_lab = self.loss_labels(outputs, targets, indices, num_boxes)
        loss_box = self.loss_boxes(outputs, targets, indices, num_boxes)

        loss_dict = {}
        # Compute all the requested losses
        for loss in self.loss_names:
            loss = self.get_loss(loss, outputs, targets, indices, num_boxes)
